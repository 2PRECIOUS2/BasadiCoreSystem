...
async function testProductMaterials() {
  ...
  try {
    ...
    console.log(`Found ${recipeResult.rows.length} products with materials:`);
    recipeResult.rows.forEach(row => {
      console.log(`  - ${row.product_name}: ${row.material_count} materials (${row.source_type})`);
    });
    ...
    consistencyResult.rows.forEach(row => {
      console.log(`  - ${row.product_name} uses ${row.measurement} ${row.unit} of ${row.material_name} @ R${row.current_price}/${row.unit} (${row.source_type})`);
    });

    // Test 4: Show any products without materials
    console.log('\n4️⃣ Checking products without materials:');
    const missingQuery = `
      SELECT 
        p.product_name,
        p.created_at
    ...
    console.log(`Found ${missingResult.rows.length} products without materials:`);
